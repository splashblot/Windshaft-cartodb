...
LayergroupController.prototype.analysisNodeStatus = function(req, res, next) {
    this.analysisStatusBackend.getNodeStatus(res.locals, (err, nodeStatus, stats) => {
...
LayergroupController.prototype.dataview = function(req, res, next) {
    const { locals } = res;
    const { user } = locals;
    const mapConfigProvider = new MapStoreMapConfigProvider(this.mapStore, user, this.userLimitsApi, locals);

    this.dataviewBackend.getDataview(mapConfigProvider, user, locals, (err, dataview, stats = {}) => {
        req.profiler.add(stats);
...
LayergroupController.prototype.dataviewSearch = function(req, res, next) {
    ...
    this.dataviewBackend.search(mapConfigProvider, user, dataviewName, locals, (err, searchResult, stats = {}) => {
        req.profiler.add(stats);
        ...
        return this.sendResponse(req, res, searchResult, 200);
    });
};
...
LayergroupController.prototype.attributes = function(req, res, next) {
    ...
    this.attributesBackend.getFeatureAttributes(mapConfigProvider, locals, false, (err, tile, stats = {}) => {
        req.profiler.add(stats);
...
LayergroupController.prototype.tileOrLayer = function (req, res, next) {
    const { locals } = res;
    const { user } = locals;
    const provider = new MapStoreMapConfigProvider(this.mapStore, user, this.userLimitsApi, locals);

    this.tileBackend.getTile(provider, req.params, (err, tile, headers, stats) => {
        req.profiler.add(stats);
        this.finalizeGetTileOrGrid(err, req, res, tile, headers, next);
    });
};
...
LayergroupController.prototype.staticMap = function(req, res, width, height, zoom /* bounds */, center, next) {
    ...
    locals.layer = locals.layer || 'all';

    const handleImage = (err, image, headers, stats) => {
    ...
    if (center) {
        this.previewBackend.getImage(mapConfigProvider, format, width, height, zoom, center, handleImage);
    } else {
        this.previewBackend.getImage(mapConfigProvider, format, width, height, zoom /* bounds */, handleImage);
    }
};

LayergroupController.prototype.sendResponse = function(req, res, body, status, headers) {
    const { locals } = res;
    const { user, token, cache_buster, dbname } = locals;
    ...
    if (cache_buster) {
        ...
        lastUpdated = new Date(parseInt(cache_buster));
    } else {
    ...
    this.getAffectedTables(user, dbname, token, (err, affectedTables) => {
        ...
        this.send(req, res, body, status, headers);
    });
};
...
LayergroupController.prototype.getAffectedTables = function(user, dbName, layergroupId, callback) {
    ...
    this._getQueriesFromMapConfig(layergroupId, (err, sql) => {
        if (err) {
            return callback(err);
        }

        this._getAffectedTablesFromQueries(user, sql, (err, tables) => {
            if (err) {
                return callback(err);
            }
...
LayergroupController.prototype._getQueriesFromMapConfig = function (layergroupId, callback) {
...
LayergroupController.prototype._getAffectedTablesFromQueries = function (user, sql, callback) {
    this.pgConnection.getConnection(user, (err, connection) => {
        if (err) {
            return callback(err);
        }

        QueryTables.getAffectedTablesFromQuery(connection, sql, callback);
    });
};
